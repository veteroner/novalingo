import type { ConversationScenario } from '../../../types/conversationScenario';

export const myFavoriteAnimalScenario: ConversationScenario = {
  id: 'phase1_animals_my_favorite_animal',
  version: 2,
  phase: 'phase1',
  title: 'My Favorite Animal',
  titleTr: 'En Sevdigim Hayvan',
  summary: 'Nova and the child talk about their favorite animals and give reasons.',
  summaryTr: 'Nova ve cocuk favori hayvanlarini konusur ve neden sevdiklerini soyler.',
  theme: 'animals',
  subTheme: 'favorite_animal',
  tags: ['animals', 'favorites', 'because'],
  ageBand: '7_9',
  difficulty: 'core',
  mode: 'semi_open',
  energy: 'playful',
  estimatedDurationSec: 200,
  turnCount: 10,
  sceneEmoji: '🦁',
  targetWords: ['lion', 'rabbit', 'bird', 'cute', 'fast', 'big', 'fun'],
  targetPatterns: ['My favorite ... is ...', 'I like ... because ...', 'It is...'],
  learningGoals: ['say a favorite', 'give a reason', 'describe an animal'],
  successCriteria: {
    minimumAcceptedTurns: 6,
    minimumTargetWordsHit: 3,
    requiredPatterns: ['My favorite ... is ...'],
    allowCompletionOnHintedAnswer: true,
  },
  reward: { rewardType: 'praise' },
  selection: {
    priority: 92,
    repeatCooldownDays: 4,
    preferredIfTagsSeen: ['animals', 'favorites'],
    avoidIfCompletedRecently: true,
  },
  variants: [
    { id: 'default', label: 'Default', labelTr: 'Varsayilan', promptStyle: 'default' },
    { id: 'short', label: 'Short', labelTr: 'Kisa', promptStyle: 'short' },
  ],
  entryNodeId: 'n1',
  nodes: [
    {
      id: 'n1',
      speaker: 'nova',
      role: 'friend',
      text: 'I love animals! Do you have a favorite animal?',
      textTr: 'Hayvanlari cok severim! Favori bir hayvanin var mi?',
      goalType: 'answer',
      targetPattern: 'My favorite ... is ...',
      hint: {
        delayMs: 9000,
        text: 'Try saying: My favorite animal is a rabbit.',
        textTr: 'Soylemeyi dene: My favorite animal is a rabbit.',
        revealPattern: true,
      },
      repair: {
        enabled: true,
        prompt: 'Say: My favorite animal is a rabbit.',
        promptTr: 'Soyle: My favorite animal is a rabbit.',
        maxRetries: 2,
      },
      responses: [
        {
          id: 'r1_lion',
          expectedText: 'My favorite animal is a lion.',
          expectedTextTr: 'En sevdigim hayvan aslan.',
          acceptedVariants: ['lion', 'favorite lion', 'my favorite animal is lion', 'my favorite animal is a lion'],
          acceptedWords: ['lion'],
          nextNodeId: 'n2_lion',
          emoji: '🦁',
          marksTargetWord: ['lion'],
          marksPattern: ['My favorite ... is ...'],
        },
        {
          id: 'r1_rabbit',
          expectedText: 'My favorite animal is a rabbit.',
          expectedTextTr: 'En sevdigim hayvan tavsan.',
          acceptedVariants: ['rabbit', 'favorite rabbit', 'my favorite animal is rabbit', 'my favorite animal is a rabbit'],
          acceptedWords: ['rabbit'],
          nextNodeId: 'n2_rabbit',
          emoji: '🐰',
          marksTargetWord: ['rabbit'],
          marksPattern: ['My favorite ... is ...'],
        },
        {
          id: 'r1_bird',
          expectedText: 'My favorite animal is a bird.',
          expectedTextTr: 'En sevdigim hayvan kus.',
          acceptedVariants: ['bird', 'favorite bird', 'my favorite animal is bird', 'my favorite animal is a bird'],
          acceptedWords: ['bird'],
          nextNodeId: 'n2_bird',
          emoji: '🐦',
          marksTargetWord: ['bird'],
          marksPattern: ['My favorite ... is ...'],
        },
      ],
    },
    {
      id: 'n2_lion',
      speaker: 'nova',
      role: 'friend',
      text: 'A lion! Wow! Lions are so strong.',
      textTr: 'Bir aslan! Vay! Aslanlar cok guclu.',
      next: 'n3',
      emoji: '🦁',
    },
    {
      id: 'n2_rabbit',
      speaker: 'nova',
      role: 'friend',
      text: 'A rabbit! So cute! I love rabbits too.',
      textTr: 'Bir tavsan! Cok sirin! Ben de tavsanlari severim.',
      next: 'n3',
      emoji: '🐰',
    },
    {
      id: 'n2_bird',
      speaker: 'nova',
      role: 'friend',
      text: 'A bird! Birds can fly. That is amazing!',
      textTr: 'Bir kus! Kuslar ucabilir. Bu muhtesem!',
      next: 'n3',
      emoji: '🐦',
    },
    {
      id: 'n3',
      speaker: 'nova',
      role: 'guide',
      text: 'Can you say it with me? Say: My favorite animal is...',
      textTr: 'Benimle soyle bilir misin? Soyle: My favorite animal is...',
      goalType: 'answer',
      targetPattern: 'My favorite ... is ...',
      hint: {
        delayMs: 7000,
        text: 'Try saying: My favorite animal is a lion.',
        textTr: 'Soylemeyi dene: My favorite animal is a lion.',
        revealPattern: true,
      },
      repair: {
        enabled: true,
        prompt: 'Say: My favorite animal is a lion.',
        promptTr: 'Soyle: My favorite animal is a lion.',
        maxRetries: 2,
      },
      responses: [
        {
          id: 'r3_lion',
          expectedText: 'My favorite animal is a lion.',
          expectedTextTr: 'En sevdigim hayvan aslan.',
          acceptedVariants: ['lion', 'my favorite animal is lion', 'my favorite animal is a lion', 'favorite lion'],
          acceptedWords: ['lion'],
          nextNodeId: 'n4',
          emoji: '🦁',
          marksTargetWord: ['lion'],
          marksPattern: ['My favorite ... is ...'],
        },
        {
          id: 'r3_rabbit',
          expectedText: 'My favorite animal is a rabbit.',
          expectedTextTr: 'En sevdigim hayvan tavsan.',
          acceptedVariants: ['rabbit', 'my favorite animal is rabbit', 'my favorite animal is a rabbit', 'favorite rabbit'],
          acceptedWords: ['rabbit'],
          nextNodeId: 'n4',
          emoji: '🐰',
          marksTargetWord: ['rabbit'],
          marksPattern: ['My favorite ... is ...'],
        },
        {
          id: 'r3_bird',
          expectedText: 'My favorite animal is a bird.',
          expectedTextTr: 'En sevdigim hayvan kus.',
          acceptedVariants: ['bird', 'my favorite animal is bird', 'my favorite animal is a bird', 'favorite bird'],
          acceptedWords: ['bird'],
          nextNodeId: 'n4',
          emoji: '🐦',
          marksTargetWord: ['bird'],
          marksPattern: ['My favorite ... is ...'],
        },
      ],
    },
    {
      id: 'n4',
      speaker: 'nova',
      role: 'friend',
      text: 'Great! Now why do you like it?',
      textTr: 'Harika! Peki neden onu seviyorsun?',
      next: 'n5',
      emoji: '💬',
    },
    {
      id: 'n5',
      speaker: 'nova',
      role: 'guide',
      text: 'Say: I like it because it is cute. Or: I like it because it is fast.',
      textTr: 'Soyle: I like it because it is cute. Ya da: I like it because it is fast.',
      goalType: 'answer',
      targetPattern: 'I like ... because ...',
      hint: {
        delayMs: 7000,
        text: 'Try saying: I like it because it is cute.',
        textTr: 'Soylemeyi dene: I like it because it is cute.',
        revealPattern: true,
      },
      repair: {
        enabled: true,
        prompt: 'Say: I like it because it is cute.',
        promptTr: 'Soyle: I like it because it is cute.',
        maxRetries: 2,
      },
      responses: [
        {
          id: 'r5_cute',
          expectedText: 'I like it because it is cute.',
          expectedTextTr: 'Onu seviyorum cunku cok tatli.',
          acceptedVariants: ['cute', 'because it is cute', 'i like it because it is cute', 'because cute'],
          acceptedWords: ['cute'],
          nextNodeId: 'n6',
          emoji: '🥰',
          marksTargetWord: ['cute'],
          marksPattern: ['I like ... because ...'],
        },
        {
          id: 'r5_fast',
          expectedText: 'I like it because it is fast.',
          expectedTextTr: 'Onu seviyorum cunku hizli.',
          acceptedVariants: ['fast', 'because it is fast', 'i like it because it is fast', 'because fast'],
          acceptedWords: ['fast'],
          nextNodeId: 'n6',
          emoji: '⚡',
          marksTargetWord: ['fast'],
          marksPattern: ['I like ... because ...'],
        },
        {
          id: 'r5_big',
          expectedText: 'I like it because it is big.',
          expectedTextTr: 'Onu seviyorum cunku buyuk.',
          acceptedVariants: ['big', 'because it is big', 'i like it because it is big', 'because big'],
          acceptedWords: ['big'],
          nextNodeId: 'n6',
          emoji: '💪',
          marksTargetWord: ['big'],
          marksPattern: ['I like ... because ...'],
        },
        {
          id: 'r5_fun',
          expectedText: 'I like it because it is fun.',
          expectedTextTr: 'Onu seviyorum cunku eglenceli.',
          acceptedVariants: ['fun', 'because it is fun', 'i like it because it is fun', 'because fun'],
          acceptedWords: ['fun'],
          nextNodeId: 'n6',
          emoji: '🎉',
          marksTargetWord: ['fun'],
          marksPattern: ['I like ... because ...'],
        },
      ],
    },
    {
      id: 'n6',
      speaker: 'nova',
      role: 'friend',
      text: 'That is a great reason! Let me think about my favorite animal...',
      textTr: 'Bu cok guzel bir neden! Ben de favori hayvanimi dusuneyim...',
      next: 'n7',
      emoji: '🤔',
    },
    {
      id: 'n7',
      speaker: 'nova',
      role: 'guide',
      text: 'My favorite animal is a dog! Tell me: Is a dog big or small?',
      textTr: 'Benim favori hayvanim kopek! Soyle bana: Bir kopek buyuk mu kucuk mu?',
      goalType: 'answer',
      targetPattern: 'It is...',
      hint: {
        delayMs: 7000,
        text: 'Try saying: It is big.',
        textTr: 'Soylemeyi dene: It is big.',
        revealPattern: true,
      },
      repair: {
        enabled: true,
        prompt: 'Say: It is big.',
        promptTr: 'Soyle: It is big.',
        maxRetries: 2,
      },
      responses: [
        {
          id: 'r7_big',
          expectedText: 'It is big.',
          expectedTextTr: 'Buyuk.',
          acceptedVariants: ['big', 'it is big', 'its big'],
          acceptedWords: ['big'],
          nextNodeId: 'n8',
          emoji: '🐕',
          marksTargetWord: ['big'],
          marksPattern: ['It is...'],
        },
        {
          id: 'r7_small',
          expectedText: 'It is small.',
          expectedTextTr: 'Kucuk.',
          acceptedVariants: ['small', 'it is small', 'its small'],
          acceptedWords: ['small'],
          nextNodeId: 'n8',
          emoji: '🐕',
          marksTargetWord: ['small'],
          marksPattern: ['It is...'],
        },
      ],
    },
    {
      id: 'n8',
      speaker: 'nova',
      role: 'friend',
      text: 'Yes! Dogs can be big or small. I love them all!',
      textTr: 'Evet! Kopekler buyuk de kucuk de olabilir. Hepsini severim!',
      next: 'n9',
      emoji: '🐾',
    },
    {
      id: 'n9',
      speaker: 'nova',
      role: 'guide',
      text: 'One last thing! Say: I love animals!',
      textTr: 'Son bir sey! Soyle: I love animals!',
      goalType: 'answer',
      targetPattern: 'I love...',
      hint: {
        delayMs: 7000,
        text: 'Say: I love animals!',
        textTr: 'Soyle: I love animals!',
        revealPattern: true,
      },
      repair: {
        enabled: true,
        prompt: 'Say: I love animals!',
        promptTr: 'Soyle: I love animals!',
        maxRetries: 2,
      },
      responses: [
        {
          id: 'r9_love',
          expectedText: 'I love animals!',
          expectedTextTr: 'Hayvanlari severim!',
          acceptedVariants: ['i love animals', 'love animals', 'animals'],
          nextNodeId: 'n10',
          emoji: '❤️',
          marksTargetWord: [],
          marksPattern: [],
        },
      ],
    },
    {
      id: 'n10',
      speaker: 'nova',
      role: 'friend',
      text: 'Me too! Animals are wonderful. You are a great animal expert!',
      textTr: 'Ben de! Hayvanlar muhtesem. Sen gercekten iyi bir hayvan uzmanisin!',
      emoji: '🌟',
    },
  ],
};
